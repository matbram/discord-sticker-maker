<script>
  ...
  function buildOutputs() {
    ...
    if (selected.gif) list.push({ type: 'gif', gif_quality: params.gif_quality })
    return list
  }
  $: anySelected = selected.sticker || selected.emoji || selected.gif

  function create() { if (!anySelected) return; view = 'working'; run() }
  ...
  function setFit(m) { params.fit_mode = m; scheduleRegen() }
  function setPriority(p) { params.priority = p; scheduleRegen() }
  function setGifQuality(q) { params.gif_quality = q; scheduleRegen() }
  function toggleBg() { params.remove_bg = !params.remove_bg; scheduleRegen() }
    ...
    <section class="studio">
      ...
      <div class="source-panel">
        ...
        {#if sourceUrl && sourceW > 0}
          ...
          <p class="muted-line">Framing applies to the sticker & emoji.</p>
        {/if}
        <div class="seg small">
        ...
        {#if sourceIsVideo}<div class="sub-card"><h4>Pick the moment</h4>
          <Trim src={sourceUrl} start={params.trim_start_s} length={params.max_duration_s} on:change={onTrimChange} /></div>{/if}

        <div class="bg-seg seg small">
        ...
        {#if outputs.length > 1}<button class="primary-btn big" on:click={downloadAll} disabled={busy}>⬇ Download all ({outputs.length})</button>{/if}
      </div>
      ...
      <div class="outputs">
        ...
        {#each TYPES as t}
          {#if selected[t.id]}
            {@const o = getOut(t.id)}
            <div class="out-card">
              <div class="out-head"><span class="t-emoji">{t.emoji}</span><b>{t.label}</b>
                {#if o}<span class="out-meta">{o.meta.width}×{o.meta.height} · {o.format} · {fmtBytes(o.meta.bytes)}{o.meta.animated ? ` · ${o.meta.frames}f` : ''}</span>{/if}
              </div>
              <div class="stage" class:dark={previewBg === 'dark'} class:light={previewBg === 'light'} class:checker={previewBg === 'checker'}>
                {#if o}<img class="sticker" class:emoji={t.id === 'emoji'} class:gif={t.id === 'gif'} src={previewUrl(t.id)} alt={t.label} />{:else}<div class="ph">…</div>{/if}
              </div>
              {#if o}
                {#if t.id === 'gif'}
                  <div class="seg three">
                ...
                {/if}
                <ul class="checks">
                  {#each Object.entries(o.meta.checklist) as [label, ok]}<li class:ok>{ok ? '✓' : '✕'} {label}</li>{/each}
                </ul>
                {#if o.meta.notes?.length}<div class="onote">{o.meta.notes[0]}</div>{/if}
                <button class="primary-btn" on:click={() => download(t.id)} disabled={busy}>⬇ Download {t.label}</button>
                <details class="howto-mini"><summary>How to add to Discord</summary>
                  <ol>{#each guidance[t.id] as step}<li>{step}</li>{/each}</ol></details>
              {/if}
            </div>
          {/if}
        {/each}
...
<style>
  ...
  .studio { display: grid; grid-template-columns: 340px 1fr; gap: 22px; margin-top: 24px; align-items: start; }
  @media (max-width: 900px) { .studio { grid-template-columns: 1fr; } }
  .source-panel { background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius-lg); padding: 18px; display: flex; flex-direction: column; gap: 12px; position: sticky; top: 16px; }
  .source-panel h3 { margin: 0; font-size: 15px; }
  .source-panel :global(.crop) { margin: 0 auto; }
  .muted-line { color: var(--muted-2); font-size: 12px; margin: 0; text-align: center; }
  .sub-card { background: var(--bg-elevated); border-radius: var(--radius); padding: 12px; }
  .sub-card h4 { margin: 0 0 8px; font-size: 13px; }
  ...
  .mini-toggle { flex: 1; min-width: 90px; padding: 8px; border-radius: var(--radius-sm); border: 1px solid var(--border); background: var(--bg-elevated); color: var(--muted); font-size: 13px; font-weight: 600; }
  ...
  .stage { position: relative; border-radius: var(--radius); display: grid; place-items: center; padding: 18px; min-height: 180px; }
  .stage.dark { background: #313338; } .stage.light { background: #fff; }
  .stage.checker { background-color: #2b2d31; background-image: linear-gradient(45deg, #3a3c42 25%, transparent 25%), linear-gradient(-45deg, #3a3c42 25%, transparent 25%), linear-gradient(45deg, transparent 75%, #3a3c42 75%), linear-gradient(-45deg, transparent 75%, #3a3c42 75%); background-size: 18px 18px; background-position: 0 0, 0 9px, 9px -9px, -9px 0; }
  .sticker { width: 160px; height: 160px; object-fit: contain; max-width: 100%; }
  .sticker.emoji { width: 48px; height: 48px; }
  .sticker.gif { width: auto; height: auto; max-width: 100%; max-height: 200px; }
